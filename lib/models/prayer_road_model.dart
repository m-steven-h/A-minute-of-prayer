// models/prayer_road_model.dart
import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';
import '../services/audio_service.dart';

// نموذج ليوم في طريق الصلاة
class PrayerDay {
  final int dayNumber;
  final String title;
  final String verse;
...
class PrayerRoadModel extends ChangeNotifier {
  ...
  /// الوقت المتبقي لفتح اليوم التالي
  ...
  /// هل ننتظر فتح اليوم التالي؟
  bool get isWaitingForNextDayToOpen {
    if (_currentDay >= 60) return false;
    // الشرط: اليوم الحالي مكتمل، واليوم التالي ليس مفتوحاً، وله وقت محدد للفتح
  ...
  String getNotificationTitle() {
    ...
    return '🎉 مبروك! أكملت اليوم $_currentDay';
  }
  ...
  String getNotificationBody() {
    ...
    if (!isCurrentDayCompleted) {
      switch (part) {
        ...
        default:
          return 'لا تنسى نصيبك اليوم في طريق الصلاة';
      }
    }
  ...
  /// حساب وقت الفتح التالي: الساعة 1:00 ظهراً بتوقيت مصر
  DateTime _calculateNextOpenTime() {
    final now = DateTime.now();

    // إنشاء وقت الفتح المستهدف (الساعة 13:00 من اليوم)
    DateTime targetTime = DateTime(now.year, now.month, now.day, 13, 0, 0);

    // إذا كان الوقت الحالي بعد الساعة 1 ظهراً، ننتقل إلى الغد
  ...
  /// التحقق من فتح اليوم التالي (يتم استدعاؤها بشكل دوري)
  Future<bool> _checkAndOpenNextDay() async {
    if (_currentDay >= 60) return false;

    // إذا كان اليوم الحالي غير مكتمل، لا نفتح اليوم التالي
    if (!_completedDays.contains(_currentDay)) return false;

    final nextDay = _currentDay + 1;

    // إذا كان اليوم التالي مفتوحاً بالفعل
    if (_openDays.contains(nextDay)) return false;

    // إذا لم يتم تحديد وقت للفتح بعد، نقوم بحسابه
    ...
    // إذا حان الوقت لفتح اليوم الجديد
  ...
  Future<void> loadData() async {
    ...
    // تصحيح: إذا كان اليوم الحالي مكتملاً واليوم التالي موجود، ننتقل تلقائياً
  ...
  /// فتح اليوم التالي يدوياً (للتجربة أو التصحيح)
  ...
  // دالة لتنظيف الموارد عند الحاجة
  void dispose() {
